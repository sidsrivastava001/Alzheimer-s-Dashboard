...
function populateDropdown() {
    ...
    if (currentUser != null && currentUser != undefined){
        ...
        firebase.database().ref("Doctors/" + email1 + "/Patients").once('value').then(function(snapshot) {
            const data1 = snapshot.val();
            for (i in data1) {
                patientEmails.push(data1[i]);
            ...
            firebase.database().ref("Patients").on('value', function(snapshot) {
                ...
                if (emailsPopulated) {
                    ...
                    removeAllChildNodes(pageSubmenu);
                    for (var n = 0; n < patientEmails.length; n++) {
